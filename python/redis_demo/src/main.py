...
class RecvedServer(threading.Thread):
    ...
    def run(self):
        redis_sub = redis.subscribeChannel('/liwei/000')  # 调用订阅方法
        while True:
...
def add_task():
    ...
    redis.publishChannel('/liwei/000', s)  # 发布
...
def main():
    try:
        ...
        #                              name='redis_recved_data_server', args=('redis_recved_data_server', taskQueue))
        #         t.start()

        recver = RecvedServer(taskQueue, 'recver')
        recver.start()

        worker = Worker(taskQueue, 'worker')
        worker.start()
